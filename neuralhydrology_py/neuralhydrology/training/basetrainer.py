...
class BaseTrainer(object):
    ...
    def train_and_validate(self):
        ...
        print(f"🚨 Starting from epoch {self._epoch + 1} to {self._epoch + self.cfg.epochs}") ##
    ...
    def _create_folder_structure(self):
        ...
        # New training run
            ...
            run_name = f"{self.cfg.experiment_name}_{timestamp}"
            
            # If run_dir is already a full path, do not append another name
            if self.cfg.run_dir is None:
                self.cfg.run_dir = Path().cwd()/"runs"/run_name
            elif not self.cfg.run_dir.name.endswith(timestamp):
