...
class NWM3Retro(BaseDataset):
    ...
    def __init__(self,
        ...
        super(NWM3Retro, self).__init__(cfg=cfg,
                                             ...
                                             id_to_int=id_to_int,
                                             scaler=scaler)
         # ✅ force load static attributes here
        self.static_attributes = self._load_attributes()
        

    def _load_basin_data(self, basin: str) -> pd.DataFrame:
        """Load input and output data. """
        df = load_timeseries(data_dir=self.cfg.data_dir, basin=basin)
            # Final check to ensure clean datetime index
    ...
    def _load_attributes(self) -> pd.DataFrame:
        self.static_attributes = load_attributes(self.cfg.data_dir, basins=self.basins)
        # IMPORTANT: Set index name explicitly
        self.static_attributes.index.name = "basin"

        return self.static_attributes
...
def load_attributes(data_dir: Path, basins: List[str] = None) -> pd.DataFrame:
    ...
    dfs = []
    for f in files:
        df = pd.read_csv(f)  # make sure we read the basin id as str
        df[df.columns[0]] = df[df.columns[0]].astype(str).str.zfill(8)
    ...
    if basins:
        ...
        # missing = set(basins) - set(df.index)
        # if missing:
        #     raise ValueError(f'Some basins are missing static attributes: {missing}')

        # # Reorder + restrict
        # df = df.loc[[b for b in basins if b in df.index]]
...
def load_timeseries(data_dir: Path, basin: str) -> pd.DataFrame:
    ...
    ds = xarray.open_dataset(netcdf_file[0])
    ds = ds.rename({'time':'date'})
    vars = ['ids', 'ids_d', 'Time_u', 'Time_d']
    for i in vars:
        if i in ds.dims:
            ds = ds.drop_vars
    # ds = ds.drop_vars(['ids', 'ids_d', 'Time_u', 'Time_d'])
    if "catchment-id" in ds.dims:
        ds = ds.squeeze("catchment-id", drop=True)

    ds = ds.assign_coords(date=pd.to_datetime(ds["date"].values)) #, origin="1979-10-01", unit="h"))
    return ds.to_dataframe()

    # Convert time to pandas datetime
    # if "date" in xr.coords:
    #     xr = xr.assign_coords(date=pd.to_datetime(xr["date"].values))
    # else:
    #     raise ValueError("No 'date' coordinate found after renaming.")

    # # Convert to dataframe and reset index
    # xr = xr.to_dataframe().reset_index()

    # # Ensure 'date' is datetime and set as index
    # xr['date'] = pd.to_datetime(xr['date'])
    # xr = xr.set_index('date')
