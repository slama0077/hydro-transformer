...
def nnse(obs: DataArray, sim: DataArray) -> float:
    r"""Normalized Nash–Sutcliffe Efficiency.

    Defined as:  NNSE = 1 / (2 - NSE)
    Maps NSE from (-inf, 1] to (0, 1], where 1 is perfect.

    Returns NaN if NSE is NaN; clips to [0, 1] for numerical stability.
    """
    # reuse the existing NSE computation (it already validates & masks)
    nse_val = nse(obs, sim)

    if not np.isfinite(nse_val):
        return float("nan")

    # protect against division by zero if NSE -> 2 (shouldn't happen, but be safe)
    denom = 2.0 - float(nse_val)
    if np.isclose(denom, 0.0):
        return 1.0

    nnse_val = 1.0 / denom

    # optional: clip to [0, 1] to avoid tiny numerical overshoots
    return float(np.clip(nnse_val, 0.0, 1.0))
...
def mae(obs: DataArray, sim: DataArray) -> float:
    r"""Calculate mean absolute error (MAE).
    
    .. math:: \text{MAE} = \frac{1}{T}\sum_{t=1}^T \left| \widehat{y}_t - y_t \right|
    ...
    return float(np.abs(sim - obs).mean())
...
def calculate_metrics(obs: DataArray,
    ...
    for metric in metrics:
        ...
        elif metric.lower() == "nnse":
            values["NNSE"] = nnse(obs, sim)
        elif metric.lower() == "mse":
            values["MSE"] = mse(obs, sim)
        elif metric.lower() == "rmse":
            values["RMSE"] = rmse(obs, sim)
        elif metric.lower() == "mae":
            values["MAE"]  = mae(obs, sim)
